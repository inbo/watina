...
eval_xg3_series <- function(data,
  ...
  file.create(tmpf)
  capture.output(
    { # to suppress the many disc_ks_test() messages
      xg3_series_props <-
        series_memberyrs %>%
        group_by(
          .data$loc_code,
  ...
  xg3_series_se <-
    ...
    summarise(
      ...
      ser_se_6y = (var(.data$value) / 6 *
        ...
        sqrt(),
      ser_rel_sd_lcl = ifelse(
        str_detect(first(.data$xg3_variable), "_lcl"),
  ...
  xg3_series_props <-
    ...
    left_join(
      xg3_series_se,
      by = c("loc_code", "xg3_variable", "series")
    )
...
date, lab_sample_id, chem_variable, value, unit, below_loq."
  ...
  if (!missing(chem_var)) {
    ...
    if (length(new) > 0) {
      warning(
        "You specified chemical variables not present in data: ",
        paste(new, collapse = ", "), "."
      )
    }
  }
  ...
  if (type != "num") {
    ...
    firstyear <- chem_qualified$date %>%
      year() %>%
      min()
    lastyear <- chem_qualified$date %>%
      year() %>%
      max()
    ...
    chem_avail <-
      ...
      mutate(
        ...
        firstdate = as_date(ifelse(
          .data$nrdates > 0,
          min(.data$date[.data$available]),
          NA
        )),
        lastdate = as_date(ifelse(
          .data$nrdates > 0,
      ...
      (function(df, unif = uniformity_test) {
        ...
        if (unif) {
          ...
          df1 %>%
            left_join(df2, by = c("loc_code", "chem_variable")) %>%
            return()
        } else {
          return(df1)
        }
  ...
  if (type != "avail") {
    chem_num <-
      ...
      summarise(
        ...
        val_geometric_mean = ifelse(
          all(.data$value > 0),
          exp(mean(log(.data$value))),
          NA
        ),
        unit = first(.data$unit),
        prop_below_loq = ifelse(
          sum(!is.na(.data$below_loq)) > 0,
          sum(.data$below_loq[!is.na(.data$below_loq)]) / sum(!is.na(.data$below_loq)),
          NA
        )
      ) %>%
      ungroup()
  ...
  chem_eval <-
    switch(type,
      ...
      "num" = chem_num,
      "both" = chem_avail %>%
        left_join(chem_num, by = c("loc_code", "chem_variable"))
    )
