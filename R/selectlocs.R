...
selectlocs_xg3 <- function(data,
  ...
  if (cond_has_avail) {
    ...
    combined_result_avail <-
      ...
      summarise(
        cond_met_avail = all(.data$cond_met),
        pct_cond_met_avail =
  ...
  if (cond_has_ser) {
    ...
    combined_result_series_xg3var <-
      ...
      summarise(
        all_ser_cond_met = all(.data$cond_met),
        pct_cond_met = round(100 * sum(.data$cond_met) / n(), 0)
      ) %>%
      # group_by(loc_code, xg3_variable) %>%  # is already done
      summarise(
        all_ser_cond_met_xg3var = any(.data$all_ser_cond_met),
        ...
        avg_pct_cond_met_nonpassed_series =
          round(
            crossprod(!.data$all_ser_cond_met, .data$pct_cond_met) /
              sum(!.data$all_ser_cond_met),
            0
          )
      )
    ...
    combined_result_series <-
      ...
      summarise(
        cond_met_series = all(.data$all_ser_cond_met_xg3var),
  ...
  if (!list) {
    return(combined_result_filtered)
  }
  ...
  if (list) {
    result <-
      if (cond_has_avail & cond_has_ser) {
        list(
          combined_result_filtered = combined_result_filtered,
...
selectlocs_chem <- function(data,
  ...
  selectlocs(
    ...
    eval_args = list(
      ...
      chem_var = chem_var,
      type = if (
        any(str_detect(conditions$statistic, "^val_")) |
          "prop_below_loq" %in% conditions$statistic
      ) {
        if (
          sum(str_detect(conditions$statistic, "^val_") |
            conditions$statistic == "prop_below_loq") < nrow(conditions)
        ) {
          "both"
        } else {
          "num"
...
selectlocs <- function(data,
  ...
  combined_result <-
    ...
    group_by(.data$loc_code) %>%
    summarise(
      all_cond_met = all(.data$cond_met),
      pct_cond_met =
        round(100 * sum(.data$cond_met) / n(), 0)
    )
