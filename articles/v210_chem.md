...
Let’s have a look at how chemical data, retrieved from Watina, look
like. The below example requests available hydrochemical data since 2010
from locations in the area ‘Zwarte Beek’:
...
- both N-NO3 (nitrate nitrogen) and P-PO4 (orthophosphate phosphorus)
  have at least two measurement dates;
- the first date of P-PO4 is not later than 1/1/2014;
- the first and last year with P-PO4 data are at least 4 years apart,
  i.e. they span at least 5 calendar years
...
We can store specific conditions in a data frame; the available
...
with the `chem_var` argument in the
...
With the argument `list = TRUE` you can also obtain intermediate test
results. For further information, see the documentation of the
[`selectlocs_chem()`](https://inbo.github.io/watina/reference/selectlocs_chem.md)
function.

## The basics: obtaining hydrochemical data

The [`get_chem()`](https://inbo.github.io/watina/reference/get_chem.md)
function in the above example retrieved hydrochemical data, from given
locations, as a lazy object (unless `collect = TRUE`). A timeframe is
used to filter hydrochemical data in Watina (with arguments `startdate`
and `enddate`). By default, the `enddate` argument is set as *today*.
...
settings for filtering according to *electroneutrality*! The default
range of electroneutrality is from -0.1 to 0.1 (argument `en_range`).
...
With `en_exclude_na = TRUE`, samples for which no electroneutrality
could be calculated are discarded!

By default, all measurements from water samples with a high iron /
conductivity ratio are returned (hence, in these cases the `en_range`
and `en_exclude_na` arguments are ignored). This is controlled by the
`en_fecond_threshold` argument. If you want the `en_range` and
`en_exclude_na` arguments to take effect also in these relatively
iron-rich water samples, set `en_fecond_threshold = NA`:
...
You can characterize the locations of a dataset of hydrochemical data,
using the
[`eval_chem()`](https://inbo.github.io/watina/reference/eval_chem.md)
function. You are invited to read its documentation and try its
examples!

The
[`selectlocs_chem()`](https://inbo.github.io/watina/reference/selectlocs_chem.md)
function, of which we saw a demonstration above, calls
[`eval_chem()`](https://inbo.github.io/watina/reference/eval_chem.md) by
itself. Alternatively the user can provide the result of those functions
as input to
[`selectlocs_chem()`](https://inbo.github.io/watina/reference/selectlocs_chem.md).
