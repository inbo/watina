...
``` r
# Run all tests created in tests/testthat
devtools::test()

# If you only want to run the snapshot tests
devtools::test(filter = "get")

# If you are verifying a data warehouse migration
...
``` r
# Optional `filter = <test-name>` to single out testing file
devtools::test()
```
